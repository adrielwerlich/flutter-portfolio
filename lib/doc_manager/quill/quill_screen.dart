...
class QuillScreen extends StatefulWidget {
  ...
  static const routeName = '/quill-doc';
    ...
    return Scaffold(
      appBar: AppBar(
        title: GestureDetector(
          onTap: () {
            showDialog(
              ...
              builder: (BuildContext context) {
                String newText = appState.quillDoc?.title ?? 'Flutter Quill';
                return AlertDialog(
                  ...
                  content: TextField(
                      ...
                      onSubmitted: (value) {
                        setState(() {
                          appState.setQuillDocTitle(newText);
                          
                          QuillDoc.updateQuillDocState(appState.quillDoc!.id, newText, null);
                        });
                        Navigator.of(context).pop();
                      },
                      controller: TextEditingController(text: appState.quillDoc?.title),
                    ),
                    actions: [
                      TextButton(
                        onPressed: () {
          ...
          child: Text(appState.quillDoc?.title ?? 'Flutter Quill'),
        ),
        actions: [
          IconButton(
